...
export default function AdminTestimonials() {
  ...
  // Use memory cache
  const { data, isLoading, updateCache } = useCache('testimonials_data', '/cms/testimonials');
  const testimonials = data || [];

  const refreshData = async () => {
    try {
      const res = await api.fetch('/cms/testimonials');
  ...
  const confirmDelete = async () => {
    ...
    try {
      await api.fetch(`/cms/testimonials/${itemToDelete.id}`, {
        method: 'DELETE',
      });
      refreshData();
